...
import com.meshnetwork.meshnetworks.ext.addOrLookupNetwork
import com.meshnetwork.meshnetworks.ext.addressToDotNotation
import com.meshnetwork.meshnetworks.ext.bssidDataStore
import com.meshnetwork.meshnetworks.ext.firstOrNull
import com.meshnetwork.meshnetworks.ext.requireHostAddress
import com.meshnetwork.meshnetworks.log.MNetLogger
import com.meshnetwork.meshnetworks.util.findFreePort
...
import com.meshnetwork.meshnetworks.vnet.socket.ChainSocketFactory
import com.meshnetwork.meshnetworks.vnet.socket.ChainSocketServer
import com.meshnetwork.meshnetworks.vnet.wifi.meshnetworksWifiManagerAndroid.OnNewWifiConnectionListener
import com.meshnetwork.meshnetworks.vnet.wifi.state.meshnetworksWifiState
import com.meshnetwork.meshnetworks.vnet.wifi.state.WifiStationState
 ...
 *
 */
class meshnetworksWifiManagerAndroid(
...
) : Closeable, meshnetworksWifiManager {

    private val logPrefix = "[meshnetworksWifiManagerAndroid: ${localNodeAddr.addressToDotNotation()}] "
    ...
    private val _state = MutableStateFlow(meshnetworksWifiState(
    ...
    override val state: Flow<meshnetworksWifiState> = _state.asStateFlow()
    ...
    init {
        ...
        wifiLock = wifiManager.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "meshnetworks").also {
            it.acquire()
        }
    ...
    suspend fun lookupStoredBssid(ssid: String) : String? {
        ...
        return appContext.bssidDataStore.data.map {
            ...
            logger(Log.DEBUG, "meshnetworksWifiManagerAndroid: lookupStoredBssid ssid=$ssid bssid=$it")
        }

    }

    suspend fun storeBssidForAddress(ssid: String, bssid: String) {
        logger(Log.DEBUG, "meshnetworksWifiManagerAndroid: storeBssidForAddress ssid=$ssid bssid=$bssid")
        val prefKey = stringPreferencesKey("${PREFIX_SSID}$ssid")
        appContext.bssidDataStore.edit {
            it[prefKey] = bssid
        }
        logger(Log.DEBUG, "meshnetworksWifiManagerAndroid: storeBssidForAddress ssid=$ssid bssid=$bssid : Done")
    }
