package com.meshnetwork.meshnetworks.client
...
import com.meshnetwork.meshnetworks.util.matchesMask
...
class UuidAllocationClient(
    ...
    private val lockByRemote = ConcurrentHashMap<com.meshnetwork.meshnetworks.RemoteEndpoint, Mutex>()
        ...
        private fun onCharacteristicReadCompat(
            ...
            if(characteristic != null && characteristic.uuid == characteristicUuid) {
                ...
                if(value != null && status == BluetoothGatt.GATT_SUCCESS) {
                    try {
                        val uuid = com.meshnetwork.meshnetworks.UuidUtil.uuidFromBytes(value)
    ...
    suspend fun requestUuidAllocation(
        ...
        val getDataPortMutex = mapLock.withLock {
            lockByRemote.getOrPut(
                com.meshnetwork.meshnetworks.RemoteEndpoint(remoteAddress, uuidMask)
            ) {
                Mutex()
            }
        }
